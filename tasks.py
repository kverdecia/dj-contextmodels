...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-contextmodels runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-contextmodels.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ contextmodels")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-contextmodels tests")
...
def release(c, bumpsize=''):
    ...
    import dj_contextmodels
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_contextmodels.__version__))
    c.run("git push --tags")
